...
import { Plus, Minus, Pencil, Check, X, PlusCircle } from "lucide-react";
...
export function PrayerCard({
  ...
  const handleQuickAdd = () => {
    ...
    if (!isNaN(amount)) {
      onSetCount(Math.max(0, Math.min(completed + amount, total)));
    }
      ...
      >
        ...
        <div className="p-6 flex flex-col items-center">
          ...
          <div className="w-full grid grid-cols-2 gap-4 mb-6">
            ...
            <div
              className="bg-muted/50 rounded-xl p-3 text-center cursor-pointer hover:bg-muted/80 transition-colors group relative min-h-[5.5rem] flex flex-col items-center justify-center"
            ...
            >
              ...
              {isEditing ? (
                ...
                <div className="flex items-center justify-center gap-1">
                  <p className="text-xl font-bold text-primary">{completed.toLocaleString()}</p>
                  <div className="flex items-center gap-2 transition-all">
                    <button
                      onClick={(e) => { e.stopPropagation(); handleStartEdit(); }}
                      className="p-2 rounded-lg bg-muted/50 hover:bg-primary/10 hover:text-primary transition-colors touch-manipulation"
                      title={t.editCount}
                    >
        ...
        <DialogContent className="rounded-2xl max-w-[320px]">
          ...
          <div className="py-4">
            <input
              ...
              className="w-full text-center text-2xl font-bold bg-background border-2 border-border rounded-xl p-3 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all"
              placeholder="0"
            />
          </div>
